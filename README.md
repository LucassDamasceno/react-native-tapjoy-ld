...
2. Go to `node_modules` ➜ `react-native-tapjoy-ld` ➜ `ios` ➜ select `RNTapjoy.xcodeproj`
...
```
  ...
  pod 'RNTapjoy', :path => '../node_modules/react-native-tapjoy-ld'
  ...
```
...
1. Add the following lines to `android/settings.gradle`:
    ```gradle
    include ':react-native-tapjoy-ld'
    project(':react-native-tapjoy-ld').projectDir = new File(rootProject.projectDir, '../node_modules/react-native-tapjoy-ld/android')
...
4. Add the compile line to the dependencies in `android/app/build.gradle`:
    ```gradle
    dependencies {
        compile project(':react-native-tapjoy-ld')
    }
    ```
